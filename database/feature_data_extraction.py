...
v1_feature_data_series: dict[str, pd.Series] = {}
v1_amr_class_counts: dict[str, int] = {}
with pathlib.Path(DB_V1).open(encoding="utf-8") as handle:
    for record in FastaIO.FastaIterator(handle):
        ...
        match amr_class:
            ...
            case "macrolide-lincosamide-streptogramin":
                amr_class = "MLS"
            case "chloramphenicol":
                amr_class = "chloramphenicol-phenicol"
            case "quinolone":
                amr_class = "quinolone-fluoroquinolone"
            case "rifampin":
                amr_class = "rifampin-rifamycin"
            case "fusidic_acid":
                amr_class = "fusidic-acid"
...
v1_cluster_sizes: dict[int, int] = {}
v1_cluster_amr_counts: dict[str, int] = {}
with pathlib.Path(CLSTR_V1).open(encoding="utf-8") as handle:
    curr_clstr_idx = -1
    for line in handle:
        if line[0] == ">":
            curr_clstr_idx += 1
        ...
        v1_feature_data_series[seq_id].at["v1-only cluster index"] = (
            curr_clstr_idx
        )
        v1_cluster_sizes[curr_clstr_idx] += 1
        clstr_amr = (
            f"{curr_clstr_idx}|{v1_feature_data_series[seq_id].at['amr class']}"
        )
...
v1_domain_counts: dict[str, int] = {}
v1_domain_amr_counts: dict[str, int] = {}
v1_superfamily_counts: dict[str, int] = {}
v1_superfamily_amr_counts: dict[str, int] = {}
for part in range(1, 31):
    with pathlib.Path(f"{CDD_V1_DIR}Part{part}_hitdata.txt").open(
        encoding="utf-8"
    ) as handle:
        prev_seq = ""
        for line in handle:
            ...
            sup = (
                row_fields[10] if row_fields[1] == "specific" else row_fields[7]
            )
            sup_amr = f"{sup}|{v1_feature_data_series[seq_id].at['amr class']}"
            if prev_seq == seq_id:
                v1_feature_data_series[seq_id].at[
                    "conserved domain(s) id(s)"
                ] = "$".join([
                    v1_feature_data_series[seq_id].at[
                        "conserved domain(s) id(s)"
                ...
                v1_feature_data_series[seq_id].at["superfamily(ies) id(s)"] = (
                    "$".join([
                        v1_feature_data_series[seq_id].at[
                            "superfamily(ies) id(s)"
                        ],
...
v2_feature_data_series: dict[str, pd.Series] = {}
v2_amr_class_counts: dict[str, int] = {}
with pathlib.Path(DB_V2).open(encoding="utf-8") as handle:
    for record in FastaIO.FastaIterator(handle):
        ...
        match amr_class:
            ...
            case "rifamycin":
                amr_class = "rifampin-rifamycin"
            case "tetracenomycin_C":
                amr_class = "tetracenomycin"
        feature_series = pd.Series(
            data={
                "amr class": amr_class,
                "amr class v2 count": 0,
...
v2_cluster_sizes: dict[int, int] = {}
v2_cluster_amr_counts: dict[str, int] = {}
with pathlib.Path(CLSTR_V2).open(encoding="utf-8") as handle:
    curr_clstr_idx = -1
    for line in handle:
        if line[0] == ">":
            curr_clstr_idx += 1
            ...
            continue
        seq_id = line.split(">")[1].split(" ")[0][:-6]
        v2_feature_data_series[seq_id].at["v2-only cluster index"] = (
            curr_clstr_idx
        )
        v2_cluster_sizes[curr_clstr_idx] += 1
        clstr_amr = (
            f"{curr_clstr_idx}|{v2_feature_data_series[seq_id].at['amr class']}"
        )
...
v2_domain_counts: dict[str, int] = {}
v2_domain_amr_counts: dict[str, int] = {}
v2_superfamily_counts: dict[str, int] = {}
v2_superfamily_amr_counts: dict[str, int] = {}
for part in range(1, 26):
    with pathlib.Path(f"{CDD_V2_DIR}Part{part}_hitdata.txt").open(
        encoding="utf-8"
    ) as handle:
        prev_seq = ""
        for line in handle:
            ...
            if prev_seq == seq_id:
                ...
                v2_feature_data_series[seq_id].at["superfamily(ies) id(s)"] = (
                    "$".join([
                        v2_feature_data_series[seq_id].at[
                            "superfamily(ies) id(s)"
                        ],
...
v1_v2_cluster_v1_sizes: dict[int, int] = {}
v1_v2_cluster_amr_v1_counts: dict[str, int] = {}
v1_v2_cluster_v2_sizes: dict[int, int] = {}
v1_v2_cluster_amr_v2_counts: dict[str, int] = {}
with pathlib.Path(CLSTR_ALL).open(encoding="utf-8") as handle:
    curr_clstr_idx = -1
    for line in handle:
        if line[0] == ">":
            curr_clstr_idx += 1
        ...
        if ver == "v1":
            ...
            v1_feature_data_series[seq_id].at["v1-v2 cluster index"] = (
                curr_clstr_idx
            )
            ...
            v2_feature_data_series[seq_id].at["v1-v2 cluster index"] = (
                curr_clstr_idx
            )
...
v1_feature_df["v1-only cluster|amr count"] = v1_feature_df.apply(
    lambda x: v1_cluster_amr_counts[
        "|".join((str(x["v1-only cluster index"]), x["amr class"]))
    ],
    axis=1,
)
v2_feature_df["v2-only cluster|amr count"] = v2_feature_df.apply(
    lambda x: v2_cluster_amr_counts[
        "|".join((str(x["v2-only cluster index"]), x["amr class"]))
    ],
    axis=1,
)
...
# Can retrieve count from v1_v2_cluster_amr_v1_counts and v1_v2_cluster_amr_v2_counts  # noqa: E501
v1_feature_df["v1-v2 cluster|amr v1 count"] = v1_feature_df.apply(
    lambda x: v1_v2_cluster_amr_v1_counts[
        "|".join((str(x["v1-v2 cluster index"]), x["amr class"]))
    ],
    axis=1,
)
v2_feature_df["v1-v2 cluster|amr v2 count"] = v2_feature_df.apply(
    lambda x: v1_v2_cluster_amr_v2_counts[
        "|".join((str(x["v1-v2 cluster index"]), x["amr class"]))
    ],
    axis=1,
)

# Now, "corresponding v2-only cluster(s) index/indices" is undefined in the v1 feature dataframe  # noqa: E501
# And "corresponding v1-only cluster(s) index/indices" is undefined in the v2 feature dataframe  # noqa: E501
# Can retrieve index from other feature dataframe
v1_feature_df["corresponding v2-only cluster(s) index/indices"] = (
    v1_feature_df.apply(
        lambda x: "$".join(
            str(idx)
            for idx in v2_feature_df
            .loc[
...
v2_feature_df["corresponding v1-only cluster(s) index/indices"] = (
    v2_feature_df.apply(
        lambda x: "$".join(
            str(idx)
            for idx in v1_feature_df
            .loc[
...
# Now, "corresponding v2-only cluster(s) size(s)" is undefined in the v1 feature dataframe  # noqa: E501
# And "corresponding v1-only cluster(s) size(s)" is undefined in the v2 feature dataframe  # noqa: E501
# Can retrieve size from v1_cluster_sizes and v2_cluster_sizes
v1_feature_df["corresponding v2-only cluster(s) size(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v2_cluster_sizes.get(int(idx or "-1"), 0))
        for idx in x["corresponding v2-only cluster(s) index/indices"].split(
            "$"
        )
    ),
    axis=1,
...
v2_feature_df["corresponding v1-only cluster(s) size(s)"] = v2_feature_df.apply(
    lambda x: "$".join(
        str(v1_cluster_sizes.get(int(idx or "-1"), 0))
        for idx in x["corresponding v1-only cluster(s) index/indices"].split(
            "$"
        )
    ),
    axis=1,
...
# Now, "corresponding v2-only cluster(s)|amr counts(s)" is undefined in the v1 feature dataframe  # noqa: E501
# And "corresponding v1-only cluster(s)|amr counts(s)" is undefined in the v2 feature dataframe  # noqa: E501
# Can retrieve count from v1_cluster_amr_counts and v2_cluster_amr_counts
v1_feature_df["corresponding v2-only cluster(s)|amr counts(s)"] = (
    v1_feature_df.apply(
        lambda x: "$".join(
            str(v2_cluster_amr_counts.get("|".join((idx, x["amr class"])), 0))
            for idx in x[
                "corresponding v2-only cluster(s) index/indices"
            ].split("$")
        ),
...
v2_feature_df["corresponding v1-only cluster(s)|amr counts(s)"] = (
    v2_feature_df.apply(
        lambda x: "$".join(
            str(v1_cluster_amr_counts.get("|".join((idx, x["amr class"])), 0))
            for idx in x[
                "corresponding v1-only cluster(s) index/indices"
            ].split("$")
        ),
...
# Now, "conserved domain(s) v1 count(s)" is undefined in the v1 feature dataframe  # noqa: E501
...
v1_feature_df["conserved domain(s) v1 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v1_domain_counts.get(ident, 0))
        for ident in x["conserved domain(s) id(s)"].split("$")
    ),
    axis=1,
)
v1_feature_df["conserved domain(s) v2 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v2_domain_counts.get(ident, 0))
        for ident in x["conserved domain(s) id(s)"].split("$")
    ),
    axis=1,
)
...
# Now, "conserved domain(s)|amr v1 count(s)" is undefined in the v1 feature dataframe  # noqa: E501
# And "conserved domain(s)|amr v2 count(s)" is undefined in both feature dataframes  # noqa: E501
...
v1_feature_df["conserved domain(s)|amr v1 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v1_domain_amr_counts.get("|".join((ident, x["amr class"])), 0))
        for ident in x["conserved domain(s) id(s)"].split("$")
    ),
    axis=1,
)
v1_feature_df["conserved domain(s)|amr v2 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v2_domain_amr_counts.get("|".join((ident, x["amr class"])), 0))
        for ident in x["conserved domain(s) id(s)"].split("$")
    ),
    axis=1,
)
...
v1_feature_df["superfamily(ies) v1 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v1_superfamily_counts.get(ident, 0))
        for ident in x["superfamily(ies) id(s)"].split("$")
    ),
    axis=1,
)
v1_feature_df["superfamily(ies) v2 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v2_superfamily_counts.get(ident, 0))
        for ident in x["superfamily(ies) id(s)"].split("$")
    ),
    axis=1,
)
...
# Now, "superfamily(ies)|amr v1 count(s)" is undefined in the v1 feature dataframe  # noqa: E501
# And "superfamily(ies)|amr v2 count(s)" is undefined in both feature dataframes
# Can retrieve count from v1_superfamily_amr_counts and v2_superfamily_amr_counts  # noqa: E501
v1_feature_df["superfamily(ies)|amr v1 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v1_superfamily_amr_counts.get("|".join((ident, x["amr class"])), 0))
        for ident in x["superfamily(ies) id(s)"].split("$")
    ),
    axis=1,
)
v1_feature_df["superfamily(ies)|amr v2 count(s)"] = v1_feature_df.apply(
    lambda x: "$".join(
        str(v2_superfamily_amr_counts.get("|".join((ident, x["amr class"])), 0))
        for ident in x["superfamily(ies) id(s)"].split("$")
    ),
    axis=1,
)
