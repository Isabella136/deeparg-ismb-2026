...
for (group, indiv) in [("Diamond AMR", "DeepARG AMR"), ("DeepARG AMR", "Diamond AMR")]:
    ...
    if group == "Diamond AMR":
        ...
        ax.set_xticks(ax.get_xticks(), ax.get_xticklabels(), rotation_mode="anchor", rotation=45, ha='left')
        ax.set_xlabel(r'$\underline{\textbf{\textsf{\Huge{Diamond\ AMR}}}}$', color="#3dac87", usetex=True)

    # Change y-axis location and label
    ax.yaxis.tick_right()
    ax.set_yticks(ax.get_yticks(), ax.get_yticklabels(), rotation_mode="anchor", rotation=0)
    ...
    for y_loc, y_group in zip(y_boundaries, y_groups.index):
        ax.text(-0.5, (y_loc+prev_y_loc)/2.0, y_group, ha="right")
        prev_y_loc = y_loc
    ...
    for x_loc, x_group in zip(x_boundaries, x_groups.index):
        if group == "Diamond AMR":
            ...
            ax.text((x_loc+prev_x_loc)/2.0, y_boundaries.iloc[-1] + 0.5, x_group, rotation=45, rotation_mode="anchor", ha="right")
        prev_x_loc = x_loc

    # Add group axis labels
    ax.text(-5, y_boundaries.iloc[-1]/2.0, "(Model, Alignment Identity)", rotation=90, va="center", fontsize=20)
    if group == "Diamond AMR":
        ax.text(
