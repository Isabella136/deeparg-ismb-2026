...
DIAMOND_HIT_FILE = "X.align.daa.tsv.BitScoreMatrix.json"
...
hit_counts_index = pd.MultiIndex.from_product(
    iterables=[sample_id_list, [30, 50, 80], ["LS", "SS"], [0], [0]])

hit_counts = hit_counts_index.to_frame(
    index=False,
    name=["sample id", "alignment identity", "model", "deeparg hit count", "diamond hit count"],)

hit_counts["deeparg hit count"] = hit_counts.apply(
    lambda x: len(open(
        f"{x["sample id"]}/{PATH_TO_RESULTS[x["model"]]}/arg_alignment_identity_{x["alignment identity"]}/{DEEPARG_HIT_FILE}"
    ).readlines()) - 1, axis=1)

hit_counts["diamond hit count"] = hit_counts.apply(
    lambda x: open(
        f"{x["sample id"]}/{PATH_TO_RESULTS[x["model"]]}/arg_alignment_identity_{x["alignment identity"]}/{DIAMOND_HIT_FILE}"
    ).read().count(': {'), axis=1
    )
